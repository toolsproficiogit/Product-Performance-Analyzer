...
const copyChartToClipboard = async (containerRef: React.RefObject<HTMLDivElement>) => {
  ...
  try {
    // Use html2canvas to capture the exact visual representation (including HTML legends)
    const canvas = await html2canvas(containerRef.current, {
      backgroundColor: '#ffffff', // Ensure white background
      scale: 2, // 2x scale for higher resolution/Retina quality
      logging: false,
      useCORS: true, // Helpful if fonts are loaded from CDN
      ignoreElements: (element) => {
        // Ignore any copy buttons if they somehow end up inside the ref container
...
const CopyButton = ({ onClick }: { onClick: () => Promise<void> }) => {
  ...
  return (
    <button
      ...
      title="Copy chart as image for presentation"
    >
      {status === 'copied' ? (
        <>
...
const ResultsCharts: React.FC<Props> = ({ data, currency }) => {
  ...
  // Bar Chart Data (Per Product Metrics)
    ...
    <div className="space-y-8 mb-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col">
          ...
          <div className="w-full h-[384px]" ref={investmentRef}>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col">
          ...
          <div className="w-full h-[384px]" ref={revenueRef}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={revenueData}
      ...
      {/* Per Product Metrics Bar Chart - Dual Axis */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="w-full h-[400px]" ref={metricsRef}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              data={perProductData}
              margin={{
